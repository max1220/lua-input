...
static int l_input_read(lua_State *L) {
	...
	if(read(input->fd, &data, sizeof(data)) > 0) {
		...
		// printf("fd: %d\n", input->fd);
		// printf("C:     time: %x, type: %x, code: %x, value: %x\n", data.time.tv_sec, data.type, data.value, data.code);
  ...
  while (getline(&line, &len, fp) != -1) {
		if (strlen(line) - 1 > 0) {
			...
			if (line[0] == 'I') {
				...
				sscanf(line, "N: Name=\"%s\"", name);
			} else if (line[0] == 'P') {
				sscanf(line, "P: Phys=%s", phys);
			} else if (line[0] == 'S') {
				sscanf(line, "S: Sysfs=%s", sysfs);
			} else if (line[0] == 'H') {
				sscanf(line, "H: Handlers=%254c", handlers);
				// w = strlen(line) - strlen("H: Handlers=")
